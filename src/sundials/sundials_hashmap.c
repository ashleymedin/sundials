...
SUNErrCode SUNHashMap_New(size_t capacity, SUNHashMap* map)
{
  ...
  *map = (SUNHashMap)malloc(sizeof(**map));
  ...
  SUNArrayList_SUNHashMapKeyValue buckets = SUNArrayList_SUNHashMapKeyValue_New(capacity);
  if (!buckets)
  {
    free(*map);
                          ...
                          int (*yieldfn)(int, SUNHashMapKeyValue, void*), void* ctx)
{
  ...
  for (size_t i = start; i < SUNArrayList_SUNHashMapKeyValue_Size(map->buckets); i++)
  {
    int retval = yieldfn(i, *SUNArrayList_SUNHashMapKeyValue_At(map->buckets, i), ctx);
    if (retval >= 0)
    {
                           ...
                           int (*compar)(const void*, const void*))
{
  ...
  *sorted = (SUNHashMapKeyValue*)malloc(SUNHashMap_Capacity(map) * sizeof(**sorted));
  if (!(*sorted)) { return SUN_ERR_MALLOC_FAIL; }

  /* Copy the buckets into a new array */
  for (i = 0; i < SUNHashMap_Capacity(map); i++) { (*sorted)[i] = *SUNArrayList_SUNHashMapKeyValue_At(map->buckets, i); }
